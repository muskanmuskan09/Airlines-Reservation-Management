...
def Avail_flight():
   conn=cx_Oracle.connect("C##892","892")
   cur=conn.cursor()
   print(conn.version)
...
def get_ticket():
   ...
   global atimeEntry
   global dateEntry
   headingLabel = Label(window, text="AGC Airlines-Ticket Window", font=("Impact", 20), bg='white',fg='black',
   ...
   headingLabel = Label(window, text="AGC Airlines-Ticket Window", font=("arial", 25, "bold"), bg='gray55',
                        fg='white',
...
# database part for Login window
        def Login_user():
            if usernameEntry.get() == '' or password.get() == '':
                ...
                if row == None:
                    ...
                    bgImage = ImageTk.PhotoImage(file="aeroplane5.jpg")
                    ...
                    heading = Label(airline, text='Welcome to AGC Airlines', font=("Impact", 30, "bold underline"),
                                    fg='black', bg="steelBlue2")
                    heading.place(x=400, y=10)


                    Button_1 = Button(airline, text='Available flights', font=('monotype 12 bold'), bg='steelBlue2',
                                      fg='black', activebackground='white', activeforeground='black',
                                      command=Avail_flight)
                    Button_1.place(x=500, y=170, width=270, height=50)
                    Button_2 = Button(airline, text='Book flight', font=('monotype 12 bold'), bg='steelBlue2', fg='black',
                                      activebackground='white', activeforeground='black', command=book_ticket)
                    Button_2.place(x=500, y=240, width=270, height=50)
                    Button_3 = Button(airline, text='Get Ticket', font=('monotype 12 bold'), bg='steelBlue2', fg='black',
                                      activebackground='white', activeforeground='black', command=get_ticket)
                    Button_3.place(x=500, y=310, width=270, height=50)
                    Button_3 = Button(airline, text='Cancel flights', font=('monotype 12 bold'), bg='steelBlue2', fg='black',
                                      activebackground='white', activeforeground='black', command=cancel_flight)
                    Button_3.place(x=500, y=380, width=270, height=50)
        ...
        # Placing Image
        bgImage = ImageTk.PhotoImage(file="sign1.png")
    ...
    heading = Label(Signup, text='CREATE AN ACCOUNT', font=("Open Sans", 20, "bold"), bg='dodger blue', fg='white')
    ...
    UsernameEntry.place(x=520, y=205)
...
def insertadminflight():
    if NoEntry.get() == '' or nameEntry .get() == '' or SourceEntry .get() == '' or DestinationEntry.get() == '' or DtimeEntry.get() == '' or AtimeEntry.get() == '' or fareEntry.get() == '' or FseatsEntry.get()==''or  EseatsEntry.get()=='' or DateEntry.get()=='':
        ...
        conn = cx_Oracle.connect(user='C##892', password='892')
        cur = conn.cursor()
        a = f"insert into flights values('{NoEntry.get()}','{nameEntry.get()}','{SourceEntry .get()}','{DestinationEntry.get()}','{DtimeEntry.get()}','{AtimeEntry.get()}','{fareEntry.get()}','{FseatsEntry.get()}','{EseatsEntry.get()}','{DateEntry.get()}')"
        cur.execute(a)
        conn.commit()
...
def insert_flight():
    ...
    headingLabel = Label(window, text="AGC Airlines", font=("arial", 25, "bold"), bg='gray55',
                         fg='white',
                         width=24, borderwidth=4, relief="solid", height=2)
    headingLabel.place(x=770, y=50)
...
def Login():
    ...
    def Login_user():
        if usernameEntry.get() == '' or password.get() == '':
            ...
            if row == None:
                ...
                bgImage = ImageTk.PhotoImage(file="v5.jpg")
                bgLabel = Label(win1, image=bgImage)
                bgLabel.place(x=0, y=0)
                frame2 = Frame(win1, bg="light blue", highlightthickness=4)
    ...
    # Placing Image
    bgImage = ImageTk.PhotoImage(file="g1.jpg")
    bgLabel = Label(LoginPage, image=bgImage)
    bgLabel.place(x=0, y=0)
    ...
    Frame_login1 = Frame(LoginPage, bg="turquoise4" ,highlightthickness=2)
    Frame_login1.place(x=580, y=0, height=700, width=1500)

    Frame_login = Frame(LoginPage, bg="white", highlightbackground='turquoise4', highlightthickness=2)
...
air.title("Airlines")
bgImage = ImageTk.PhotoImage(file="v1.jpg")
...
bgLabel.place(x=0, y=0)
heading = Label(air, text='AGC Airline', font=("Impact", 30, "bold"), bg='white', fg='dodger blue')
