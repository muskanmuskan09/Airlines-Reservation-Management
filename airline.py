...
def display():
    conn = cx_Oracle.connect(user='airline', password='mmmk')
    cur = conn.cursor()
    query = "select pass_no from passenger where pass_no=:1"
    ...
    if row == None:
          ...
          query = "select pass_name from passenger where pass_no=:1"
          cur.execute(query, (IDEntry.get(),))
          rows = cur.fetchone()
          nameEntry.insert(0,rows)
          nameEntry.config(state="disabled")

          a = f"Select flight_no from passenger where pass_no=:1"
          cur.execute(a, (IDEntry.get(),))
          rows = cur.fetchone()
          flightnoEntry.insert(0, rows)
          flightnoEntry.config(state="disabled")

          a = f"Select class from passenger where pass_no=:1"
          cur.execute(a,(IDEntry.get(),))
          rows = cur.fetchone()
          ...
          a=f"Select flights.flightname from flights inner join passenger on flights.flightno=passenger.flight_no where pass_no=:1"
          cur.execute(a,(IDEntry.get(),))
          rows = cur.fetchone()
          flight_nameEntry.insert(0, rows)
          flight_nameEntry.config(state="disabled")

          a = f"Select flights.source from flights inner join passenger on flights.flightno=passenger.flight_no where pass_no=:1"
          cur.execute(a,(IDEntry.get(),))
          rows = cur.fetchone()
          sourceEntry.insert(0, rows)
          sourceEntry.config(state="disabled")

          a = f"Select flights.destination from flights inner join passenger on flights.flightno=passenger.flight_no where pass_no=:1"
          cur.execute(a,(IDEntry.get(),))
          rows = cur.fetchone()
          destinationEntry .insert(0, rows)
          destinationEntry.config(state="disabled")

          a = f"Select flights.departuretime from flights inner join passenger on flights.flightno=passenger.flight_no where pass_no=:1"
          cur.execute(a,(IDEntry.get(),))
          rows = cur.fetchone()
          dtimeEntry.insert(0, rows)
          dtimeEntry.config(state="disabled")

          a = f"Select flights.arrivaltime from flights inner join passenger on flights.flightno=passenger.flight_no where pass_no=:1"
          cur.execute(a,(IDEntry.get(),))
          rows = cur.fetchone()
          atimeEntry.insert(0, rows)
          atimeEntry.config(state="disabled")

          a = f"Select flights.f_date from flights inner join passenger on flights.flightno=passenger.flight_no where pass_no=:1"
          cur.execute(a, (IDEntry.get(),))
          rows = cur.fetchone()
...
def cancelflight():
   conn = cx_Oracle.connect(user='airline', password='mmmk')
   cur = conn.cursor()
   query2="Select pass_no from passenger where pass_no=:1 and pass_name=:2"
   ...
   if passnoEntry.get()=='' or passnameEntry.get()=='':
    ...
    if row == None:
      ...
      query1 = "Delete from passenger where pass_no=:1"
